...
export async function POST(req: NextRequest) {
    try {
        ...
        const { importe, concepto, docNro, docTipo, email, nombre, items } = body;
        ...
        return NextResponse.json({ error: error.message || "Error al generar factura", details: error.toString() }, { status: 500 });
    }
}
